"""APScheduler wiring: outbox dispatch and cleanup jobs."""
...
logger = logging.getLogger("user-service.tasks")


def build_scheduler(session_factory, publisher) -> AsyncIOScheduler:
    """Build the user-service scheduler. ``publisher`` may be None when RabbitMQ
    is unavailable; the outbox job tolerates that and relies on retries."""
